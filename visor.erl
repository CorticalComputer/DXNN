...
loop()->
	...
	{Scape_PId,ScapeType} = gen_server:call(polis,{get_scape,ScapeType}),
...
draw_avatar(Canvas,Avatar)->
	...
	%{actuator,Actuator,ActuatorId,Parameters} = Tag, [{CF_PId,CFVL,CFTag}|CF]
		CF = Avatar#avatar.actuators,
		CT = Avatar#avatar.sensors,
		Loc = Avatar#avatar.loc,
		%io:format("draw_stats:CF:~p CT:~p Loc:~p~n",[CF,CT,Loc]),
